...
// --- APPRENTISSAGE IA : LECTURE DES MOTS INCONNUS ---
export async function setupAdminAIPage() {
    const listContainer = document.getElementById('admin-ai-list');
    if (!listContainer) return;

    listContainer.innerHTML = '<tr><td colspan="4" class="p-6 text-center text-gray-500"><i data-lucide="loader-2" class="w-6 h-6 animate-spin mx-auto"></i></td></tr>';
    if (window.lucide) window.lucide.createIcons();
    ...
    try {
        ...
        console.error("Erreur chargement IA unknowns:", e);
        listContainer.innerHTML = '<tr><td colspan="4" class="p-6 text-center text-red-500">Erreur de chargement.</td></tr>';
    }
}
...
window.exitImmersionMode = function() {
    ...
    loadBoutiquesList();
    setupAdminAccessPage();
    if(window.setupAdminAIPage) window.setupAdminAIPage();
